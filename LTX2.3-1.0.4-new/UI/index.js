...
// 动态获取当前访问的域名或 IP，自动对齐 3000 端口
    ...
    async function pollQueueStatus() {
        ...
        try {
            ...
            if (!data?.current && Array.isArray(data?.history)) {
                const latestComplete = data.history.find(task => task && task.status === 'complete');
                if (latestComplete && latestComplete.id !== lastAutoDisplayedQueueTaskId) {
                    const rawPath = latestComplete.result?.image_paths?.[0] || latestComplete.result?.video_path;
                    const outputType = latestComplete.result?.image_paths?.length ? 'image' : 'video';
                    if (rawPath) {
                        lastAutoDisplayedQueueTaskId = latestComplete.id;
                        try { displayOutput(rawPath, outputType, { silentLog: true }); } catch (_) {}
                    }
                }
            }
        } catch (_) {}
