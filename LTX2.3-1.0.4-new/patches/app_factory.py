    ...
    *,
    ...
    def _normalize_queue_result(result) -> dict:
        ...
        view: dict[str, object] = {
            "status": getattr(result, "status", "complete") or "complete"
        }
        for attr in (
            "video_path",
            "image_paths",
        ...
        for raw_path in _result_media_paths(task.get("result")):
            try:
                ...
                if str(raw_path).startswith("/outputs/"):
                    media_path = get_dynamic_output_path() / Path(raw_path).name
                if not media_path.is_absolute():
                    media_path = get_dynamic_output_path() / raw_path
    ...
    async def route_set_low_vram_mode(request: Request):
        ...
        from low_vram_runtime import (
            ...
            restore_full_vram_config_tweaks,
        ...
        if enabled and not should_use_cpu_offload():
            return JSONResponse(
                status_code=400,
                content={"error": "显存上限为 0/空时不会启用低显存模式"},
            )
    ...
    async def route_save_vram_limit(request: Request):
        try:
            ...
            if numeric_limit <= 0:
                ...
                handler.pipelines.low_vram_mode = False
                write_low_vram_pref(False)
                restore_full_vram_config_tweaks(handler)
                
            return {
                "status": "ok",
                "vramLimit": limit_text,
                "lowVramMode": bool(getattr(handler.pipelines, "low_vram_mode", False)),
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
    ...
    def patched_generate(self, req: GenerateVideoRequest):
        ...
        # 宽高均需为 64 的倍数（LTX 内核校验）；在近似画幅下取整
        RESOLUTION_MAP = {
            "540p": (1024, 576),
            "720p": (1280, 704),
            "1080p": (1920, 1088),
        ...
        try:
            ...
            result = patched_generate_video(
                ...
                lora_strength=float(getattr(req, "loraStrength", 2.0) or 2.0),
    ...
    async def route_generate_batch(request: Request):
        ...
        try:
            ...
            loraStrength = float(data.get("loraStrength") or 2.0)
            ...
            for idx, seg in enumerate(segments_in):
                ...
                req = GenerateVideoRequest(
                    ...
                    customWidth=data.get("customWidth"),
                    customHeight=data.get("customHeight"),
                    modelPath=modelPath,
                    loraPath=loraPath,
    ...
    async def route_get_replay(request: Request):
        try:
            ...
            if raw_path:
                ...
                media_path = get_dynamic_output_path() / filename
            if media_path is None:
                return JSONResponse(status_code=400, content={"error": "Missing path"})
            if not media_path.is_absolute():
                media_path = get_dynamic_output_path() / media_path.name
